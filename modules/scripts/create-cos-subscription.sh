...
ibmcloud_login() {
    ...
    echo "ERROR: Failed to login to IBM Cloud after ${max_attempts} attempts"
    exit 1
}
...
subscription_exists() {
    ibmcloud ce subscription cos get --name "${SUBSCRIPTION_NAME}" &>/dev/null
}

# Create COS subscription
create_subscription() {
    echo "Creating Code Engine COS subscription: ${SUBSCRIPTION_NAME}"
    echo "  Destination: ${APP_NAME} (app)"
    echo "  Bucket: ${BUCKET_NAME}"
    echo "  Event type: write"
    ...
    if ! ibmcloud ce subscription cos create \
        ...
        echo "ERROR: Failed to create COS subscription"
        exit 1
    fi
...
# Main execution
echo "Starting Code Engine COS subscription creation..."
...
if subscription_exists; then
    ...
    echo "To update the subscription, delete it first and re-run this script."
else
    create_subscription
fi
